...
# Safe display for tide times (already formatted as strings or NA)
safe_display_tide_time <- function(time_value) {
  # The tide times from get_noaa_tidal_data are already formatted strings
  # We just need to check if they're NULL, NA, or empty
  if (is.null(time_value) || is.na(time_value) || time_value == "") {
    return("N/A")
  }
  return(time_value)
}
...
generate_scenario_predictions <- function(model, current_env) {
  ...
  for (i in seq_along(beaches)) {
    ...
    status <- if (predicted_value > GEORGIA_THRESHOLD) "Advisory Likely" else "No Advisory Predicted"
    status_icon <- ""  # Removed checkmark symbols
...
tab1_ui <- tabItem(
    ...
    box(
      ...
      div(
        ...
        p(style = "margin: 0; font-size: 13px;",
          ...
          "Low confidence means conditions are uncertain - check back for updates or use caution with water contact.")
      )
    )
  ),
  ...
  fluidRow(
    box(
      width = 12,
      title = "Beach Monitoring Locations",
      status = "primary",
      solidHeader = TRUE,
      ...
      p(style = "margin-bottom: 15px; font-size: 14px;",
        "Interactive map showing the 5 monitored beach sites on Tybee Island. ",
        "Click on markers to see today's predictions for each location."),
      
      leafletOutput("tab1_beach_map", height = 450),
      
      div(
        style = "margin-top: 15px; padding: 10px; background-color: #e7f3ff; border-left: 4px solid #0066cc; border-radius: 4px;",
        p(style = "margin: 0; font-size: 13px;",
          icon("info-circle"),
          " Each beach is tested regularly. The markers show predicted water quality status based on current environmental conditions.")
      )
    )
  ),
      ...
      fluidRow(
        column(6,
               div(style = "padding: 15px;",
                   h4(icon("check"), " When No Advisory is Predicted", style = "color: #28a745;"),
                   tags$ul(
                     style = "font-size: 14px; line-height: 1.8;",
...
tab1_server <- function(input, output, session, beach_predictions, current_conditions, trained_models) {
  ...
  output$unified_weather_display <- renderUI({
    ...
    tagList(
      ...
      p(style = "margin: 5px 0;", sprintf("%.1fÂ°F", temp_f)),
  ...
  output$unified_tide_display <- renderUI({
    ...
    # Tide times are already formatted strings from get_noaa_tidal_data()
    next_high_text <- safe_display_tide_time(tide$next_high$time)
    next_low_text <- safe_display_tide_time(tide$next_low$time)
  ...
  output$unified_water_temp_display <- renderUI({
    ...
    tagList(
      ...
      p(style = "margin: 5px 0; font-size: 16px;", 
        strong(sprintf("%.1fÂ°F", water_temp$temp_f))),
      p(style = "margin: 5px 0; font-size: 14px;", sprintf("(%.1fÂ°C)", water_temp$temp_c)),
  ...
  # === BEACH LOCATIONS MAP ===
  output$tab1_beach_map <- renderLeaflet({
    req(beach_predictions())
    ...
    # Map site names to match predictions
    name_mapping <- c(
    ...
    map_data <- site_coords %>%
      left_join(preds, by = "beach_name") %>%
      filter(!is.na(prediction))
    
    # Create popups
    map_data$popup_content <- with(map_data, paste0(
      "<div style='text-align: center;'>",
      "<h4 style='margin: 5px 0;'>", beach_name, "</h4>",
      "<p style='margin: 5px 0; font-size: 16px;'><strong>", status_icon, " ", status, "</strong></p>",
      "<p style='margin: 5px 0;'>Predicted Level: <strong>", prediction, " CFU/100mL</strong></p>",
      "<p style='margin: 5px 0;'>Confidence: <strong>", confidence_level, " (", confidence_pct, "%)</strong></p>",
      "<p style='margin: 5px 0; font-size: 12px; color: #666;'>", key_factors, "</p>",
      "</div>"
    ))
    ...
    leaflet(map_data) %>%
      ...
      setView(lng = mean(map_data$Longitude), lat = mean(map_data$Latitude), zoom = 13) %>%
      addLegend(
        position = "bottomright",
        colors = c("#28a745", "#dc3545"),
        labels = c("No Advisory Predicted", "Advisory Likely"),
...
# 97% safe message - Featured prominently in header (beaches monitored)
# Language correction - "No Advisory Predicted" instead of "Safe"
# Tide times - Now displaying next high/low correctly
# 
# TO USE:
# 1. Replace your tab1_complete.R with this file
# 2. Make sure models are in models/ directory
# 3. Make sure Excel files are in www/ directory
